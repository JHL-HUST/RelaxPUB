    ...
    */
	...
	void try_color(ui &color_ub, ui lb, int c) {
		...
		if (colors[c].sz > 0) {
			...
			// color_ub = colors[c].sz < K ? colors[c].sz : K;
			// return;
	...
	void bitColorSort4() {
		...
		for (int i = PLSTART(depth); i < PLEND(depth); i++) {
			disnode.push_back(vector<int>());
			int u = stkvtx[i];
			for (int j = CASTART(depth); j < CAEND(depth); j++) {
				int v = stkvtx[j];
				if (!bg->rows[u]->test(v))
					disnode[i - PLSTART(depth)].push_back(j);
			}
		}
		...
		while (lb > 0){
			...
			float color_profit_rate = (float) (colors[c].sz) / color_ub;
			...
			if (last_pub_profit > color_profit_rate){
				for (int i = PLSTART(depth); i < PLEND(depth); i++){
					ui v = stkvtx[i];
					ui ub = K - (SZ_PLEX(depth) - con2p[depth][v]);
					//cout << i << " " << ub << "\t\t";
					if (ub == 0 || disnode[i - PLSTART(depth)].empty())
						continue;
					ui count = 0;
					for (int j : disnode[i - PLSTART(depth)])
						if (has_include2[j - CASTART(depth)] == false)
							count++;		
					//cout << i << " " << count << "\t\t";
					ui invest = count > ub ? ub : count;
					if (invest == 0)
					{
						disnode[i - PLSTART(depth)].clear();
						continue;
					}
					if (invest > lb)
						continue;
				...
				// cout << "last_pub_profit: " << last_pub_profit << " " << color_profit_rate << endl;
			}
			...
			if (color_profit_rate > max_profit_rate || (color_profit_rate == max_profit_rate && color_ub > saved_invest)){
				...
				if (investor_idx < 0)
					break;
				pub_time++;
				cbound[depth][c] = saved_invest;

				for (int j : disnode[investor_idx - PLSTART(depth)]){
					if (has_include2[j - CASTART(depth)] == false){
						has_include2[j - CASTART(depth)] = true;
						ui u = stkvtx[j];
						stkvtx_tmp[pos++] = u;
						bound[depth][u] = 0;
					}
				...
				disnode[investor_idx - PLSTART(depth)].clear();
		...
		while (r_num2 > 0) {
			...
			if (color_profit_rate > max_profit_rate || (color_profit_rate == max_profit_rate && color_ub > saved_invest)){
				...
				for (int j = 0; j < colors[c].sz; j++) {
					ui u = colors[c].vertices[j];
					stkvtx_tmp[pos++] = u;
					bound[depth][u] = j+1 < color_ub ? accbound + j+1 : accbound + color_ub;		//bound[depth][u]ָ�ľ��ǵ�u������bound		
				}
				accbound += color_ub;
				pcolors[depth][c] = pos;
				r_num2 -= (colors[c].sz);
				...
				for (int j : disnode[investor_idx - PLSTART(depth)]){
					if (has_include2[j - CASTART(depth)] == false){
						...
						ui u = stkvtx[j];
						stkvtx_tmp[pos++] = u;
						bound[depth][u] = num_tmp < saved_invest ? accbound + num_tmp : accbound + saved_invest;
					}
				}
				pcolors[depth][c] = pos;
				accbound += saved_invest;
				disnode[investor_idx - PLSTART(depth)].clear();
				r_num2 -= num_tmp;
				// lb -= saved_invest;
				// r_num -= saved_count;
