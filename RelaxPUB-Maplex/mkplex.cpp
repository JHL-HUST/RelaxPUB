...
void massiveMKplex(){
  ...
  if (kernalG.nbvtx >= 500000){
    ...
    // FILE* pp = fopen("result.xls","a+");
    // fprintf(pp,"%s\t%d\t%d\t%d\t%lld\t%lld\t%lld\t%0.3f\n",filename,paraK,optimal,szbest,fes->nnodes,color_time,pub_time,Utility::elapse_seconds(stclk, endclk));
   	// fclose(pp); 
     
	}else{
    
    // FILE* pp = fopen("result.xls","a+");
    // fprintf(pp,"%s\t%d\t%d\t0\t0\n",filename,paraK,optimal);
   	// fclose(pp); 
