...
void Graph::read_graph()
{
	...
	if (edges != nullptr)
		delete[] edges;
	edges = new ui[m];
	// pstart��¼ÿһ������vp�п�ʼ��λ�ã���Ҫ��¼ÿ������edges�ķ�Χ��
	// edges��¼��ǰvp�бߵ���һ���˵�
	pstart[0] = 0;
	ui idx = 0;
	for (ui i = 0; i < n; i++)
...
void Graph::kPlex_exact()
{
	...
	if (kplex.size() < UB)
	{
		...
		ui *Qv = new ui[n];
		ui Qv_n = 0;
		// ��һ��Լ��ͼ��ɾ��ɾ�ߣ���Ҫ���ڱ�֤��ǰͼ��С�Ѿ�����2k-1����������ͼ��
		if (kplex.size() + 1 > 2 * K)
		{
		...
		{
			// �൱����n����ͼ��������Щ��ͼ�ֱ�����ÿ����Ϊ���ĵĶ����ھ��γɵ���ͼ��Ȼ������Щ��ͼ����lb
			ui u, key;
			bool ret_tmp = linear_heap->pop_min(u, key); // u��ǰͼ�ж���С�ĵ㣬key�õ�Ķ�
			assert(ret_tmp);
			// if(key != 0) printf("u = %u, key = %u\n", u, key);
			if (key < kplex.size() + 1 - K) // �Բ�����������Ƶĵ�
			{
#ifndef NDEBUG
			...
			if (kplex.size() + 1 >= 2 * K)
			{
				// �൱�ڵ�ǰlb����2k-1��Ҫ���Ժ����Ҷ�����ͼ��ͬʱ����ɾ�ߺ�lookahead
			...
			if (ids_n > kplex.size()) // ����ͼ�������е����kplex���ж��Ƿ��ܳ���lb
			{
				double density = (double(vp.size() * 2)) / ids_n / (ids_n - 1);
				...
				if (ids_n > max_n_search)
					max_n_search = ids_n;
				kplex_solver->load_graph(ids_n, vp); // ��ͼ���½�ͼ
#ifdef NAIVE
...
void Graph::extract_subgraph(ui u, ui *ids, ui &ids_n, ui *rid, vector<pair<int, int>> &vp, char *exists, ept *pstart, ept *pend, ui *edges, char *deleted, ui *edgelist_pointer)
{
	...
	for (ui i = 0; i < old_size; i++)
	{
		...
		for (ept j = pstart[u]; j < pend[u]; j++)
			if (edges[j] > u)
			{
				vp.push_back(make_pair(rid[u], rid[edges[j]])); // ��u�Ķ����ھ��γɵ���ͼ�еı߱��������Ŵ�0��ʼ��ԭʼ��ű�����ids��
			}
	}
...
void Graph::heuristic_kplex_max_degree(ui processed_threshold)
{
	Timer t;
	assert(kplex.empty());
	ui *head = new ui[n]; // head��next�ֱ��¼ͬһdegree��ʼ�ĵ����һ��ͬһdegree�ĵ�
	ui *next = new ui[n];
	ui *degree = new ui[n]; // ��¼��
...
void Graph::shrink_graph(ui &n, ept &m, ui *peel_sequence, ui *core, ui *out_mapping, ui *in_mapping, ui *rid, ept *pstart, ui *edges, bool output)
{
	...
	// rid��֪��ԭ���������ţ���out_mapping��֪���������ԭ��ţ��ֱ��Ƕ�core�еĵ�����ģ�ȥ�����Ժ��һ���µ���ţ�
	if (cnt != n)
	{
		cnt = 0;
		ept pos = 0;
...
void Graph::oriented_triangle_counting(ui n, ui m, ui *peel_sequence, ept *pstart, ept *pend, ui *edges, ui *tri_cnt, ui *adj)
{
	ui *rid = adj;
	for (ui i = 0; i < n; i++)
		// ��Žڵ���peel_sequence�е�λ�ã�ps:peel_sequence�ǰ��������
...
void Graph::reorganize_oriented_graph(ui n, ui *tri_cnt, ui *edge_list, ept *pstart, ept *pend, ept *pend2, ui *edges, ui *edgelist_pointer, ui *buf)
{
	for (ui i = 0; i < n; i++)
		pend2[i] = pend[i];
	ept pos = 0;
	//	��֮ǰɾ���ı߼ӻ�����֮ǰֻ�����Ӷ�С�ĵ��ȴ�ıߣ��ּӻ�������Ȼ��edgelist_pointer��Ϊָʾ�ñ���edge_list�е�λ��
	for (ui i = 0; i < n; i++)
	{
...
//peeling(n, linear_heap, Qv, Qv_n, kplex.size() + 1 - K, Qe, true, kplex.size() + 1 - 2 * K, tri_cnt, active_edgelist, active_edgelist_n, edge_list, edgelist_pointer, deleted, degree, pstart, pend, edges, exists);
